...
| `Zy789kl/moonlabelkit/agreement` | Cohen's Kappa, Fleiss' Kappa 与 Krippendorff's Alpha | `cohen_kappa.mbt`, `fleiss_kappa.mbt`, `alpha.mbt`, `confusion_matrix.mbt`, `agreement_test.mbt` | 440 |
...
| `Zy789kl/moonlabelkit/report` | HTML 格式数据治理检查报告与样式渲染器 | `html.mbt` | 120 |
...
4. **权威的标注一致性量化评估 (`agreement`)**：
   ...
   - 实现高级 **Krippendorff's Alpha ($\alpha$)**，针对多标注者与缺失数据提供极其严谨的学术级置信度检验。
5. **立体的质量缺陷自动化巡检 (`quality` & `report`)**：
...
### 5.5 仓库自检与 CI
仓库已补齐 GitHub Actions 持续集成，工作流位于 [`.github/workflows/ci.yml`](.github/workflows/ci.yml)。
